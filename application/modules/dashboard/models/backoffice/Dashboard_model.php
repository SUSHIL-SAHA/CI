...
class Dashboard_model extends BaseModel {
    ...
    public function activeportfolio($data,$id){ 
        $this->db->where('id', $id);
        $this->db->update(TABLE_PREFIX.'user_portfolio', $data); 
    }
	...
	$this->load->helper('date');
		...
		/* fname checking */
		$regex_text = '/^[a-zA-ZÄäÀàÁáÂâÃãÅåǍǎĄąĂăÆæĀāÇçĆćĈĉČčĎđĐďðÈèÉéÊêËëĚěĘęĖėĒēĜĝĢģĞğĤĥÌìÍíÎîÏïıĪīĮįĴĵĶķĹĺĻļŁłĽľÑñŃńŇňŅņÖöÒòÓóÔôÕõŐőØøŒœŔŕŘřẞßŚśŜŝŞşŠšȘșŤťŢţÞþȚțÜüÙùÚúÛûŰűŨũŲųŮůŪūŴŵÝýŸÿŶŷŹźŽžŻż ]*$/';
		if($fname==''){
			$msg['status']='FAIL';
			$msg['errors'][$i]['id']='fname';
			$msg['errors'][$i]['message']=__('signup_please_enter_first_name','Please enter first name');
			$i++;
		} else if(!preg_match($regex_text, $fname)){
			$msg['status']='FAIL';
			$msg['errors'][$i]['id']='fname';
		...
		if($lname==''){
			...
			$i++;
		} else if(!preg_match($regex_text, $lname)){
			$msg['status']='FAIL';
			$msg['errors'][$i]['id']='lname';
			$msg['errors'][$i]['message']= __('signup_alphabetic','Please enter alphabetic character');
			$i++;
		...
		if($i==0){
			...
			$curr_image=$this->auto_model->getFeild('logo',TABLE_PREFIX.'user','user_id',$user_id);
			...
			$upd_user=$this->db->update(TABLE_PREFIX.'user', $data);
			if ($upd_user) {
				$msg['status'] = "OK";
				$msg['showmsgonly'] = true;
	...
	foreach($res->result() as $row){
	 ...
	 public function deleteskill($user_id){ 
	      $this->db->where('user_id', $user_id);
		 return $this->db->delete(TABLE_PREFIX.'new_user_skill'); 
	 }      
	...
	public function getUserSkills($user_id=''){
		...
		$this->db->select('ps.skill_name as parent_skill_name,s.skill_name as skill , ps.id as parent_skill_id , s.id as skill_id')
				->from(TABLE_PREFIX.'new_user_skill us')
				->join(TABLE_PREFIX.'skills ps' , 'ps.id=us.skill_id' , 'LEFT')
				->join(TABLE_PREFIX.'skills s' , 's.id=us.sub_skill_id' , 'INNER');
    ...
    public function deleteportfolio($id){ 
        ...
        $user =  $this->auto_model->getFeild("user_id",TABLE_PREFIX."user_portfolio","id",$id);
		if($user ==  $login_user_id){
			$file_name=  $this->auto_model->getFeild("original_img",TABLE_PREFIX."user_portfolio","id",$id);
			$file_name_thumb=  $this->auto_model->getFeild("thumb_img",TABLE_PREFIX."user_portfolio","id",$id);
			unlink("assets/portfolio/".$file_name);
			unlink("assets/portfolio/".$file_name_thumb);
			$this->db->delete(TABLE_PREFIX.'user_portfolio', array('id' => $id));   
		}
      ...
      public function getProject($status='',$id='')
	  {
			...
			$rs=$this->db->get(TABLE_PREFIX.'projects');
			$data=array();
          ...
          public function getportfolio($user_data,$limit = '9', $start = ''){
              ...
              $res= $this->db->get_where(TABLE_PREFIX."user_portfolio",array("user_id" => $user_data));
              $data=array();
		  ...
		  public function getActiveportfolio($user_id=''){
			 ...
			 $this->db->where('status', 'Y');
              ...
              $res= $this->db->get_where(TABLE_PREFIX."user_portfolio",array("user_id" => $user_id));
          ...
          public function count_portfolio($user_data){ 
            ...
            $this->db->from(TABLE_PREFIX.'user_portfolio');
            return $this->db->count_all_results();              
          }
          
          public function update_extendday($data,$id){ 
            $this->db->where('id', $id);
            $this->db->update(TABLE_PREFIX.'projects', $data);               
          }
          
            public function insert_notification($data){ 
              $this->db->insert(TABLE_PREFIX."notification",$data);
		  ...
		  if($user)
		  {
			  	...
			  	$answer = filter_data($data['answers']);
			  	$question_id = $data['question_id'];
			  	if($question_id==''){
					$msg['status']='FAIL';
					$msg['errors'][$i]['id']='question';
					$msg['errors'][$i]['message']= __('dashboard_setting_please_choose_your_new_security_question','Please choose your new Security Question');
					$i++;
				}	
				if($answer==''){
					$msg['status']='FAIL';
					$msg['errors'][$i]['id']='answer';
					$msg['errors'][$i]['message']= __('dashboard_setting_answer_is_required','Answer is required.');
					$i++;
				}

				if($i==0){
					$this->db->select("answers");
					$this->db->from(TABLE_PREFIX.'answers');	
					$this->db->where("user_id",$user_id);
					$res = $this->db->get();    
					if(count($res->result())>0){ 
					$this->db->query("UPDATE ".TABLE_PREFIX."answers set answers='".$answer."',question_id ='".$question_id."' ,reset_code ='".''."'WHERE user_id = '".$user_id."'");
					}else{
						$datainsert = ['answers'=>$answer,'question_id'=>$question_id,'user_id'=>$user_id];
						$this->db->insert(TABLE_PREFIX."answers",$data);		
						$msg['message']= 'Updated Successfully';
					}
					$msg['status']='OK';
					$msg['message']=__('dashboard_setting_security_question_update_success','Updated Successfully');
					$msg['location']=  SITE_URL.'dashboard/setting';
					
				}
		  }else{
			$msg['status']='FAIL';
			$msg['errors'][$i]['id']='question';
			$msg['errors'][$i]['message']= __('no_user_selected','No User Selected.');
		  }
		  echo json_encode($msg);
		}
	...
	public function insertreset_code($user_id,$code){
		...
		$update = $this->db->query("UPDATE ".TABLE_PREFIX."answers set reset_code='".$code."' WHERE user_id = '".$user_id."'");
		if($update){
				$from = $this->auto_model->getFeild('admin_mail',TABLE_PREFIX.'setting','id',1);;
		...
		public function getUpdatedAnswer(){
			...
			$result = $this->db->query("SELECT question_id FROM ".TABLE_PREFIX."answers WHERE user_id = '".$user_id."'");
			...
			if( $result->num_rows())
			{
				...
				$this->db->from(TABLE_PREFIX.'questions');	
        ...
        if($oldpass!=''){
			...
			$this->db->select("*");
			$res=$this->db->get_where(TABLE_PREFIX.'user',$con);                        
			$c= count($res->result());
        ...
        if($i==0){ 
            ...
            $this->db->where('user_id',$user_id);
            if($this->db->update(TABLE_PREFIX.'user', $data)){ 
                $msg['status']='OK';    
         ...
         public function editportfolio(){ 
		  ...
		  public function getBidder($project_id)
		  {
				...
				$this->db->from(TABLE_PREFIX.'bids');
				return $this->db->count_all_results();  
		 }	
		 public function getAllBidder($project_id)
		 {
				$this->db->select();
				$this->db->where('project_id',$project_id);
				$rs=$this->db->get(TABLE_PREFIX.'bids');
				$data=array();
				foreach($rs->result() as $row)
				{
					$fname = $this->auto_model->getFeild('fname',TABLE_PREFIX.'user','user_id',$row->bidder_id);
					$lname = $this->auto_model->getFeild('lname',TABLE_PREFIX.'user','user_id',$row->bidder_id);
		...
		public function updateProject($data,$id)
		{
			...
			return $this->db->update(TABLE_PREFIX.'projects',$data);
		} 
		...
		public function countReview($pid,$given_id,$user_id)
		{
			...
			$this->db->where('user_id',$user_id);
			$this->db->from(TABLE_PREFIX.'review');
			return $this->db->count_all_results();
		...
		public function getrating($user_id)
		{
			...
			$this->db->where('user_id',$user_id);
			$res=$this->db->get(TABLE_PREFIX.'review');
			$data=array();
			foreach($res->result() as $row)
			{
				$data[]=array(
			...
			$this->db->select_sum('average');
			$this->db->where('user_id',$user_id);
			$res=$this->db->get(TABLE_PREFIX.'referer_review');
			$new_data=array();
			foreach($res->result() as $row)
			{
				$new_data[]=array(
			...
			$this->db->select('average');
			$this->db->where('user_id',$user_id);
			$this->db->from(TABLE_PREFIX.'referer_review');
   ...
   public function closeAccount(){ 
       $user=$this->session->userdata('user');
        ...
        $this->db->where('user_id', $user[0]->user_id);
        $this->db->update(TABLE_PREFIX.'user', $data2);    
   ...
   public function insertEscrow($data)
   {
	...
	{
		$this->db->where($where);
		return $this->db->update(TABLE_PREFIX.'project_milestone',$data);	
	}
	public function updateNotification($id)
	{
		...
		return $this->db->update(TABLE_PREFIX.'notification',$data);	
	}
	...
	public function insert_notification_file($id){
              ...
              if($id>0){
              	$count_notic=$this->auto_model->count_results('id',TABLE_PREFIX.'notification','','',array('to_id'=>$id,'read_status'=>'N'));
	...
	public function getrating_new($user_id){
		...
		$res=$this->db->get(TABLE_PREFIX.'review_new');
		$data=array();
		foreach($res->result() as $row)
		{
			$data[]=array(
		...
		$this->db->select('average');
		$this->db->where('review_to_user',$user_id);
		$this->db->from(TABLE_PREFIX.'review_new');
		$data[0]['num']=$this->db->count_all_results();
	...
	public function getRecentBidProjects($user_id=''){
		$this->db->select('p.project_id,p.title,p.post_date,p.status as project_status,p.project_type, p.bidder_id as all_bidders, b.bidder_amt,b.add_date as bid_date')
			->from(TABLE_PREFIX.'bids b')
			->join(TABLE_PREFIX.'projects p', 'p.project_id=b.project_id')
	...
	public function getRecentProjects($user_id=''){
		...
		$bids = $this->db->dbprefix(TABLE_PREFIX.'bids');
	...
	public function getProjectStatics($user_id=''){
		$open_projects = $this->db->where(array('user_id' => $user_id, 'status' => 'O'))->count_all_results(TABLE_PREFIX.'projects');
		$completed_projects = $this->db->where(array('user_id' => $user_id, 'status' => 'C'))->count_all_results(TABLE_PREFIX.'projects');
		
		$active_projects = $this->db->where(array('user_id' => $user_id, 'status' => 'P'))->count_all_results(TABLE_PREFIX.'projects');
		$cnl_projects = $this->db->where(array('user_id' => $user_id, 'status' => 'CNL'))->count_all_results(TABLE_PREFIX.'projects');
	...
	public function getmyreview_new($user_id=''){
		$this->db->select("*");
		$this->db->where('review_to_user',$user_id);
		$data=$this->db->order_by('review_id', 'DESC')->get_where(TABLE_PREFIX.'review_new')->result_array();
		return $data;

	}
	
	public function getEscrowProject($user_id='', $limit=0, $offset=40, $for_list=TRUE){
		$projects = $this->db->dbprefix(TABLE_PREFIX.'projects');
		$this->db->select("p_txn.project_id, p.title, SUM( txn_row.debit) AS total_debit, SUM( txn_row.credit) AS total_credit, txn_row.wallet_id")
			->from(TABLE_PREFIX.'project_transaction p_txn') 
			->join(TABLE_PREFIX.'transaction_new txn', 'txn.txn_id=p_txn.txn_id')
			->join(TABLE_PREFIX.'transaction_row txn_row', 'txn_row.txn_id=p_txn.txn_id')
			->join(TABLE_PREFIX.'projects p', 'p.project_id=p_txn.project_id', 'LEFT')
			->where('txn.status', 'Y');
	...
	public function make_profile_feature($userid,$month){

	}


	public function getNoofQuotations(){
		$this->db->select("COUNT(service_inquiry_id) AS no_of_quotations");
	...
	public function getNoofPayments(){
		$this->db->select("COUNT(id) AS no_of_payments");
