...
class auto_model extends BaseModel {
    ...
    public function setting() {
        ...
        $this->db->select();
        $query = $this->db->get_where(TABLE_PREFIX."setting", array());
		$data=array();
    ...
    public function leftPannel() {
        ...
        $query = $this->db->get_where(TABLE_PREFIX."adminmenu", array("status" => 'Y', "parent_id" => 0));
        $i = 0;
        foreach ($query->result() as $row) {
            $i++;
            $result[$i]['id'] = $row->id;
    ...
    public function leftpanelchild($id) {
        $result = array();
        $this->db->select();
        $query = $this->db->get_where(TABLE_PREFIX."adminmenu", array("status" => 'Y', "parent_id" => $id));
        $i = 0;
        foreach ($query->result() as $row) {
            $i++;
            $result[$i]['id'] = $row->id;
    ...
    {
        $this->db->select();
        $query = $this->db->get_where(TABLE_PREFIX."adminmenu", array("status" => 'Y', "parent_id" => $id), 1,0);
        $parent_url = '';
        foreach ($query->result() as $row) {
    ...
    public function getFeild($select, $table, $feild = "", $value = "", $where = null) {
        ...
        if ($value != '' AND $feild != '') {
            ...
            $rs = $this->db->get_where(TABLE_PREFIX.$table, array($feild => $value));
            ...
            $rs = $this->db->get_where(TABLE_PREFIX.$table, $where);
        }
	...
	$this->db->select($attr);	
	$rs = $this->db->get_where(TABLE_PREFIX.$table,array($by=>$value));
	 $data = array();
	 ...
	 return $data;
       ...
       public function getskill($pid){         
        ...
        $res=$this->db->get_where(TABLE_PREFIX."skills",$con);
        $data=array();
	...
	public function getcategory($pid){         
        ...
        $res=$this->db->get_where(TABLE_PREFIX."categories",$con);
        $data=array();
   ...
   public function get_results($table_name, $query = array(), $cols = "*", $offset = '', $limit = '0', $order_by = array()) 
    {
        $this->db->select($cols)->from(TABLE_PREFIX.$table_name);
...
function check_demo_admin($redirect_url = '', $default_page = 'page') {
    ...
    function updateUser($data,$id)
    {
        ...
        return $this->db->update(TABLE_PREFIX.'admin',$data);
